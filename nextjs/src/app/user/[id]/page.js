...
export default function UserProfile() {
        ...
        return (
            <div className="min-h-screen bg-cream flex items-center justify-center font-sans-clean px-4">
                <div className="bg-cream-card rounded-[2.5rem] border border-cream-dark/80 p-8 sm:p-10 shadow-xl text-center max-w-sm w-full">
                    ...
                    <p className="text-charcoal/60 text-sm mb-6">You need to be logged in to view profiles.</p>
    ...
    const profilePhotoUrl = profileUser?.profilePhoto || `https://ui-avatars.com/api/?name=${profileUser?.username || "User"}`;
                        ...
                        {currentUser._id !== userId && (
                            <div className="flex gap-4">
                                <button 
                                    ...
                                    className={`px-8 py-3 rounded-full font-bold text-xs shadow-sm transition-all cursor-pointer border outline-none ${
                                        isFollowing 
                                ...
                                <button 
                                    ...
                                    className={`px-8 py-3 rounded-full font-bold text-xs shadow-sm transition-all flex items-center gap-2 cursor-pointer border outline-none ${
                                ...
                                {profileUser?.acceptedChallenges?.filter(c => c.status === 'active').length > 0 ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {profileUser.acceptedChallenges.filter(c => c.status === 'active').map((challenge, idx) => (
                                            <div key={challenge._id || idx} className="bg-cream-card rounded-3xl p-6 border border-cream-dark/80 shadow-md">
                                                <h3 className="text-lg font-serif-elegant text-charcoal mb-2">{challenge.challengeText}</h3>
                                                <p className="text-xs text-charcoal/40">Status: Active</p>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
