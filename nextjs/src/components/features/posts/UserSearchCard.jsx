...
export default function UserSearchCard({ user }) {
    ...
    const profilePhotoUrl = user.profilePhoto || `https://ui-avatars.com/api/?name=${user.username || "User"}`;
        ...
        >
            ...
            <div className="flex items-center gap-2">
                {user.connectionStatus === 'connected' ? (
                    ...
                    <button className="bg-charcoal hover:bg-black text-white font-bold px-4 py-1.5 rounded-full text-[10px] shadow-sm transition bg-transparent">
                        View
                    </button>
                )}
            </div>
