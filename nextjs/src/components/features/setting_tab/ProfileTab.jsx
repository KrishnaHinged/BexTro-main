...
export default function ProfileTab({ onLogout }) {
  ...
  const profilePhotoUrl = previewUrl || `https://ui-avatars.com/api/?name=${profileData.fullName || "User"}`;
      ...
      <form onSubmit={handleProfileUpdate} className="space-y-5">
        ...
        <div className="flex items-start gap-3.5 bg-indigo-50/20 p-5 rounded-3xl border border-indigo-100/50 shadow-inner">
          <input
            type="checkbox"
            id="isPrivate"
            checked={profileData.isPrivate || false}
        ...
        <div className="flex gap-3.5 pt-4">
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-3.5 bg-charcoal hover:bg-black text-white rounded-full font-semibold shadow-md text-xs transition cursor-pointer border-none outline-none"
          >
          ...
          <button
            ...
            className="px-6 py-3.5 border border-charcoal/20 hover:bg-charcoal/5 text-charcoal rounded-full font-semibold text-xs transition cursor-pointer bg-transparent"
          >
            Logout
          </button>
        </div>
